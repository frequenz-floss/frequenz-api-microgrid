...
- The `AddComponentBoundsRequest.validity_duration` field was replaced by `AugmentElectricalComponentBoundsRequest.request_lifetime` to match the fields in `SetComponentPowerActiveRequest` and `SetComponentPowerReactiveRequest` (these messages have bee removed. See below).
- The `AddComponentBoundsResponse.ts` field was renamed to `AugmentElectricalComponentBoundsResponse.valid_until_time` for extra clarity.
- The version of `frequenz-api-common` has been updated from v0.6.1 to v0.8.0. Please refer to the release notes for `frequenz-api-common` versions [v0.7.0](https://github.com/frequenz-floss/frequenz-api-common/releases/tag/v0.8.0) and [v0.8.0](https://github.com/frequenz-floss/frequenz-api-common/releases/tag/v0.8.0) and update your code accordingly. The following changes have been made in the Microgrid API to align with the changes in `frequenz-api-common`:
